...
import (
	...
	"github.com/MayurakshaSikdar/golang-tutorial/projects/project-text-speech/speech"
)
...
func makeSpeechOpt(text, voice, out string, rate, pitch float64) (*speech.SpeechOptions, error) {
	if text == "" {
	...
	case "stand-a":
		voiceName = speech.VoiceA
	case "stand-b":
		voiceName = speech.VoiceB
	case "stand-c":
		voiceName = speech.VoiceC
	case "stand-d":
		voiceName = speech.VoiceD
	case "wave-a":
